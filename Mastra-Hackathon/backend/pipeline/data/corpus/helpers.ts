...
export function seedsToEntries(seeds: ChapterSeed[]): KnowledgeEntry[] {
  return seeds.map((seed) => {
    const subjectLabel = subjectStorageLabel(seed.subjectKey);
    const prefix = `Class ${seed.classLevel} ${subjectLabel}`;
    return {
      keywords: seed.keywords,
      chunks: seed.content.map((paragraph, i) => ({
        id: `c${seed.classLevel}-${seed.subjectKey}-${slug(seed.topic)}-${i + 1}`,
...
function subjectStorageLabel(key: SubjectKey): string {
