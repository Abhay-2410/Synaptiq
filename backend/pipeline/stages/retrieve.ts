...
/**
 * Retrieve course material via @mastra/qdrant (primary).
...
export async function retrieveContext(doubt: DoubtRequest): Promise<RetrievedChunk[]> {
  ...
  try {
    fromQdrant = await retrieveFromMastraQdrant(searchDoubt);
  } catch (err) {
    if (requireLiveQdrant()) throw err;
    console.warn('[retrieve] Qdrant failed, continuing to corpus fallback:', err);
  }
