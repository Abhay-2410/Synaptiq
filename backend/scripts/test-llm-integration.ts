/**
 * Featherless LLM integration smoke test — tutor paths + reasoning evaluator calibration.
...
if (!isLlmProviderConfigured()) {
  console.error('FAIL: FEATHERLESS_API_KEY not configured');
  process.exit(1);
}
if (isLlmReachable() === false) {
  console.error('FAIL: Featherless not reachable at startup');
  process.exit(1);
}
...
console.log('=== Part 2: Quick Challenge evaluator via Featherless ===\n');
let failed = 0;
...
if (failed > 0) {
  console.error(`${failed} calibration case(s) out of range on Featherless`);
  process.exit(1);
}

console.log('PASS: Featherless integration — answers + reasoning evaluator calibration OK.');
